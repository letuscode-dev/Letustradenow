...
window.Blockly.Blocks.digit_pair_return_differs_prediction = {
    init() {
        this.jsonInit({
            ...
            tooltip: localize(
                'Learns A → B → C → D for every digit pattern, then returns stored D when A → B → C reappear as the previous three digits.'
            ),
    ...
    meta() {
        return {
            ...
            description: localize(
                'Stores the follower after A → B → C → D, then signals Digit Differs D when A → B → C appear again as the previous three digits before a new tip.'
            ),
            key_words: localize('digit pair, quartet, return, differs, pattern, confirmation'),
        };
    },
