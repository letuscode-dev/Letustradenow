/**
 * Apollo notify — same behaviour as standard notify (XML compatibility).
 */
...
window.Blockly.Blocks.apollo_notify = {
    ...
    meta() {
        return {
            display_name: localize('Notify (Apollo)'),
            description: localize('Compatibility notify block for imported Apollo strategies.'),
        };
    },
...
window.Blockly.JavaScript.javascriptGenerator.forBlock.apollo_notify = block => {
    ...
    if (message_block?.type === 'variables_get') {
        ...
        variable_name = window.Blockly.derivWorkspace.getVariableById(variable_id)?.name || null;
    }
    ...
    return `Bot.notify({ className: 'journal__text--${notificationType}', message: ${message}, sound: '${sound}', block_id: '${block.id}', variable_name: ${JSON.stringify(variable_name)} });\n`;
};
