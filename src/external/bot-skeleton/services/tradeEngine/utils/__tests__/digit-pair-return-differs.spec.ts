...
describe('digit pair return differs', () => {
    it('stores A → B → C and signals Differ C when A → B returns', () => {
        ...
        const confirmation = evaluateDigitPairReturnDiffers([7, 3, 1, 7, 3], {}, state);
    ...
    it('tracks independent pairs for all digits 0–9', () => {
        ...
        const confirmation = evaluateDigitPairReturnDiffers([0, 9, 4, 2, 5, 8, 0, 9], {}, state);
    ...
    it('bootstraps targets from an epoch-tagged rolling window without stale trades', () => {
        ...
        const confirmation = evaluateDigitPairReturnDiffers(
            [...history, { digit: 7, epoch: 7 }, { digit: 3, epoch: 8 }],
            {},
            state
        );
        expect(confirmation.prediction).toBe(1);
    ...
    it('processes only the new tip when a plain digit sliding window advances', () => {
        ...
        const next = evaluateDigitPairReturnDiffers([7, 3, 1, 7], {}, state);
        expect(next.prediction).toBe(-1);

        const confirmation = evaluateDigitPairReturnDiffers([3, 1, 7, 3], {}, state);
        expect(confirmation.prediction).toBe(1);
    });
});
