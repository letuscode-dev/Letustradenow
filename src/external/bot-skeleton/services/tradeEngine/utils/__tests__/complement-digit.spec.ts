...
describe('getComplementDigitPrediction', () => {
    it('returns previous digit for each complement pair', () => {
        expect(getComplementDigitPrediction([0, 9])).toBe(0);
        expect(getComplementDigitPrediction([9, 0])).toBe(9);
        expect(getComplementDigitPrediction([1, 8])).toBe(1);
        expect(getComplementDigitPrediction([8, 1])).toBe(8);
        expect(getComplementDigitPrediction([2, 7])).toBe(2);
        expect(getComplementDigitPrediction([7, 2])).toBe(7);
        expect(getComplementDigitPrediction([3, 6])).toBe(3);
        expect(getComplementDigitPrediction([6, 3])).toBe(6);
        expect(getComplementDigitPrediction([4, 5])).toBe(4);
        expect(getComplementDigitPrediction([5, 4])).toBe(5);
    });
    ...
    it('uses only the last two digits in a longer window', () => {
        expect(getComplementDigitPrediction([1, 2, 3, 0, 9])).toBe(0);
...
describe('evaluateComplementDigit', () => {
    it('journals a short signal line when a complement fires', () => {
        ...
        expect(result.journal_messages[0].message).toBe('Complement 4→5 · Differs 4');
    });
