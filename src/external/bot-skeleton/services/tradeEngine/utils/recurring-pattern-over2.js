/**
 * Recurring Pattern Over 2 Consistency
 *
 * Sliding digit patterns. When a pattern recurs, measure how often the next
 * digit is Over 2 (3–9) AND how consistently that behavior repeats.
 * Prior occurrences only — no look-ahead. Contract is DIGITOVER barrier 2 only.
 */

export const BASELINE_OVER2_PCT = 70;
export const OVER2_BARRIER = 2;

export const STATUS = {
    NO_COMPLETE_PATTERN: 'NO_COMPLETE_PATTERN',
    INSUFFICIENT_OCCURRENCES: 'INSUFFICIENT_OCCURRENCES',
    OVER2_RATE_TOO_LOW: 'OVER2_RATE_TOO_LOW',
...
const isOver2Win = digit => digit !== null && digit > OVER2_BARRIER;
...
export const normalizeRecurringPatternOver2Options = (options = {}) => {
    ...
    let min_over2_rate = toNonNegNumber(pick('min_over2_rate', presets.min_over2_rate), presets.min_over2_rate);
    ...
    if (mode === 'strict') {
        ...
        min_over2_rate = Math.max(min_over2_rate, STRICT_DEFAULTS.min_over2_rate);
...
const createPatternRecord = () => ({
    next_digits: [],
    outcomes: [], // 1 = Over 2 win, 0 = loss
    occurrence_indices: [],
    live: { signals: 0, wins: 0, losses: 0 },
});

export const createRecurringPatternOver2State = () => ({
    digits: [],
    absolute_index: -1,
    patterns: {},
    bootstrapped: false,
...
export const resetRecurringPatternOver2State = (state = null) => {
    const next = createRecurringPatternOver2State();
...
const classifyQuality = ({ occurrences, over2_rate, consistency_score, min_occurrences }) => {
    if (occurrences < min_occurrences) return 'INSUFFICIENT DATA';
    if (over2_rate >= 80 && consistency_score >= 80) return 'STRONG & CONSISTENT';
    if (over2_rate >= 80 && consistency_score < 70) return 'STRONG BUT UNSTABLE';
    if (over2_rate >= 70) return 'MODERATE';
    return 'WEAK';
};
...
/**
 * Consistency score 0–100 — deliberately NOT a copy of Over 2 %.
...
const computeSignalScore = ({
    ...
    const rate_part = Math.min(30, Math.max(0, (over2_rate - 70) * 2));
...
const analyzeOutcomes = (outcomes, options) => {
    ...
    const over2_rate = total ? (wins / total) * 100 : 0;
    const edge = over2_rate - BASELINE_OVER2_PCT;
    ...
    const recent_change = recent_rate - over2_rate;
...
const evaluatePatternCandidate = (state, pattern_digits, options) => {
    ...
    if (stats.total < options.min_occurrences) {
    ...
    } else if (stats.over2_rate + 1e-9 < options.min_over2_rate) {
        reasons.push(
            `REJECTED: OVER 2 RATE BELOW THRESHOLD | Observed: ${stats.over2_rate.toFixed(2)}% | Required: ${options.min_over2_rate}%`
        );
        status = STATUS.OVER2_RATE_TOO_LOW;
        ...
        reasons.push(
            `REJECTED: RECENT PERFORMANCE WEAK | Historical: ${stats.over2_rate.toFixed(2)}% | Recent: ${stats.recent_rate.toFixed(2)}% | Required: ${options.min_recent_rate}%`
        );
    ...
    return {
        ...
        reasons,
        status,
        prediction: qualified ? OVER2_BARRIER : -1,
        rejection: reasons[0] || '',
        contract_type: 'DIGITOVER',
        barrier: OVER2_BARRIER,
    };
};
...
const buildJournal = ({ best, options, matched, status, rejection, patterns_evaluated, valid_signals, cooldown }) => {
    ...
    messages.push({
        ...
        message: `══ RECURRING PATTERN OVER 2 (${String(options.mode).toUpperCase()}) ══`,
    });
    ...
    messages.push({
        className: 'journal__text',
        message: `OVER 2: Wins ${s.wins} | Losses ${s.losses} | Rate ${s.over2_rate.toFixed(2)}% | Baseline ${BASELINE_OVER2_PCT}% | Edge +${s.edge.toFixed(2)}pp`,
    });
    messages.push({
        className: 'journal__text',
    ...
    if (cooldown) {
        ...
        messages.push({
            ...
            message: `STATUS: VALID SIGNAL — ACTION: OVER ${OVER2_BARRIER}`,
        });
    } else {
        messages.push({
            className: 'journal__text',
...
const recordFollower = (state, pattern_digits, follower, end_abs) => {
    ...
    record.outcomes.push(isOver2Win(follower) ? 1 : 0);
...
export const evaluateRecurringPatternOver2 = (
    raw_ticks,
    raw_options = {},
    state = createRecurringPatternOver2State()
) => {
    const options = normalizeRecurringPatternOver2Options(raw_options);
    ...
    if (ticks.length) {
        ...
        ticks.forEach(tick => {
            ...
            if (state.pending_outcome) {
                recordRecurringPatternOver2Outcome(state, tick.digit);
            }
            ...
            // 1) Prior pattern ending at previous tip gets this tip as Over 2 outcome.
            if (idx >= 1 && idx - L >= 0) {
 ...
 */
export const replayRecurringPatternOver2 = (raw_ticks, raw_options = {}) => {
    const options = normalizeRecurringPatternOver2Options({
        ...raw_options,
    ...
    for (let i = 0; i < digits.length; i++) {
        const result = evaluateRecurringPatternOver2(digits.slice(0, i + 1), options, state);
        if (result.pattern) patterns_detected += 1;
        if (result.matched) {
            valid_signals += 1;
            consistency_sum += result.consistency_score || 0;
            over2_sum += result.over2_rate || 0;
            recent_sum += result.best?.stats?.recent_rate || 0;
            signals.push({
                tip: i,
                pattern: result.pattern,
                over2_rate: result.over2_rate,
    ...
    return {
        ...
        average_historical_over2_rate: valid_signals > 0 ? over2_sum / valid_signals : 0,
